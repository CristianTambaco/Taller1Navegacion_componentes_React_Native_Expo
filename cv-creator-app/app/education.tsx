...
export default function EducationScreen() {
  ...
  // Estado para DatePicker de año de graduación
  const [showGradYearPicker, setShowGradYearPicker] = useState(false);
  const [tempGradYear, setTempGradYear] = useState(new Date());
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        {/* DatePicker para Año de Graduación */}
        <Text style={{ marginTop: 12, marginBottom: 4 }}>Año de Graduación</Text>
        <TouchableOpacity
          style={styles.datePickerButton}
          onPress={() => setShowGradYearPicker(true)}
        >
          <Text style={styles.datePickerText}>
            {formData.graduationYear ? formData.graduationYear : "Selecciona el año"}
          </Text>
        </TouchableOpacity>
        {showGradYearPicker && (
          <DateTimePicker
            ...
            onChange={(event, selectedDate) => {
              setShowGradYearPicker(Platform.OS === "ios");
              if (selectedDate) {
                setTempGradYear(selectedDate);
                // Solo el año
                const yearStr = selectedDate.getFullYear().toString();
                setFormData((prev) => ({ ...prev, graduationYear: yearStr }));
              }
            }}
          />
        )}
